...
export default function Attendance() {
  ...
  const getEffectiveStatus = (row, dayIndex) => {
    ...
    if (row.joiningDate) {
      const cellDate = new Date(filter.year, filter.month - 1, d);
      const joinDate = new Date(row.joiningDate);
  ...
  const getShortStatus = (status, holiday) => {
    if (!status) return "-";
    if (status === 'not-joined') return 'NJ';
  ...
  const processAttendanceData = (apiRecords, leavesIn = []) => {
    ...
    employees.forEach(emp => {
      ...
      // ── JOINING DATE FILTER ──
      // If the selected month/year is before the employee's joining date (month/year), the employee's name/row must not appear in the attendance page log/table at all.
      if (emp.joiningDate) {
        const joinDate = new Date(emp.joiningDate);
        const joinMonth = joinDate.getMonth() + 1;
        const joinYear = joinDate.getFullYear();
        if (filter.year < joinYear || (filter.year === joinYear && filter.month < joinMonth)) {
          return;
        }
      }
          ...
          if (!existing) {
            ...
            // If same type, prioritize active status (Present/Half-day) over passive (Holiday/Absent)
              ...
              const priority = {
                ...
                'on-field': 7, 'leave': 6, 'holiday': 5, 'weekly-off': 4, 'absent': 1
              };
  ...
  const getStatusColor = (status, day, isTodayForCell) => {
    ...
    switch (status) {
      ...
      case 'on-field': return 'bg-cyan-100 text-cyan-600 border-cyan-200';
      case 'not-joined': return 'bg-slate-100 text-slate-400 border-slate-200 opacity-60';
      ...
      <div className={`${glassCls} border border-slate-100 flex-1 flex flex-col min-h-0 shadow-sm rounded-md`}>
              ...
              {records
                ...
                <tr key={idx} className="hover:bg-slate-50 group transition-standard">
                  ...
                  {row.days.map((day, i) => {
                    ...
                    if (status === 'not-joined') statusChar = "NJ";
                    if (status === 'on-field') statusChar = "F";

                    let hoverText = status || "";
                    if (status === 'not-joined') {
                      hoverText = "Not Joined";
                    ...
                    return (
                      <td
                        key={i}
                        className={`px-0 py-2 border-r border-slate-100 text-center align-middle transition-colors ${status === 'not-joined' ? 'cursor-not-allowed bg-slate-50/30' : 'cursor-pointer hover:bg-blue-50/50'}`}
                        onClick={() => status !== 'not-joined' && openManualMark(row.userId, i)}
                      >
                        {status ? (
                          <div
        ...
        <div className="p-3 border-t border-slate-100 bg-slate-50/30 flex flex-wrap gap-x-4 gap-y-1.5 items-center">
          {[
            ...
            { char: 'F', label: 'On Field', status: 'on-field' },
            { char: 'NJ', label: 'Not Joined', status: 'not-joined' },
          ].map((item, idx) => (
